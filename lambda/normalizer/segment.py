...
def segment(raw_text, units, registry):
    ...
    prompt = (
        ...
        "- WHERE a document ends is a question about the artefact. WHAT it is "
        "is a question about the content. The two are answered differently, "
        "and the rules below are grouped accordingly.\n"
        ...
        "\nWhat each part is:\n"
        "- Type on what the part CONTAINS and does, never on the form it "
        "arrived in. An email, a letter, a slide deck, a scan and a signed form "
        "can all be the same type. A mail thread in which one side answers "
        "questions about its own ownership and business is a questionnaire. A "
        "mail thread describing how a company sources, ships and gets paid is "
        "an operations description. Do not answer \"an email thread\" and stop "
        "there - the wrapper is never the type.\n"
        "- Read the description under each type on the list. It says what the "
        "type is FOR, and it is written to be matched against content.\n"
        "- A part need not match its type perfectly. Choose the type whose "
        "description best covers the substance of what the part holds, even "
        "where the part also carries other material. Perfect fit is not the "
        "test; best fit is.\n"
        "- Return null only when NO type on the list plausibly covers the "
        "content. Null is the right answer for genuinely unlisted material, and "
        "the wrong answer for listed material in an unexpected wrapper. Null is "
        "about the TYPE only - it is never a reason to merge parts.\n"
